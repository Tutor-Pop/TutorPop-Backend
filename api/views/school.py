...
def get_edit_delete_school(request, school_ID: int):
    if request.method == GET:
        try:
            ...
            return Response({"message": "School doesn't exist!"}, status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            ...
            for i in teachers:
                ...
                Teacher.objects.get_or_create(
                    account_id=tmpAcc, school_id=tmpSchool)
        ...
        try:
            ...
            for i in teachers:
                Teacher.objects.filter(
                    account_id=i, school_id=school_ID).delete()
            all_teachers = Teacher.objects.filter(school_id=school_ID)
            result = JSONParser(all_teachers)  # แสดงผลครูทั้งหมดในโรงเรียน
            return Response({"teachers": result}, status=status.HTTP_200_OK)
        except:
...
def edit_status_school(request, school_ID: int):
    try:
        school = School.objects.get(school_id=school_ID)
        serializer = SchoolStatusSerializer(
            school, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
        return Response(serializer.data, status=status.HTTP_200_OK)
